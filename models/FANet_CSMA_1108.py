...
#             nn.Conv2d(in_channels=128, out_channels=1, kernel_size=1), nn.Sigmoid(), )
...
#         equi_feature = self.msfusion(equi_feature)
#         feats_mutual_er, feats_mutual_cb = self.fusion_mutual_attention(equi_feature, proj_feature)
#         out = feats_mutual_er+equi_feature
#         # out = self.Conv1x1(self.msfusion(self.feature_selection(torch.cat((equi_feature, feats_mutual_er), dim=1))))
#         # out = self.Conv1x1(feats_mutual_er+equi_feature)

#         # out1 = F.relu(self.bn1(self.conv1(out)), inplace=True) #256
#         # out2 = self.conv2(out1)
#         # # w, b = out2[:, :64, :, :], out2[:, 64:, :, :]
#         # out = F.relu(out2[:, :128, :, :] * out1 + out2[:, 128:, :, :], inplace=True)
...
class FPNFusion(nn.Module):
    ...
    def forward(self, x):
        ...
        # l3Feature = l3 
        # out1 = F.relu(self.bn1(self.conv1(l3Feature)), inplace=True) #256
        # out2 = self.conv2(out1)
        # # w, b = out2[:, :64, :, :], out2[:, 64:, :, :]
        # l3Feature = F.relu(out2[:, :64, :, :] * out1 + out2[:, 64:, :, :], inplace=True)

        # l2Feature = l2 + F.interpolate(l3Feature, size=l2.size()[2:], mode='bilinear', align_corners=True)
        # # l2Feature = l2
        # out1 = F.relu(self.bn1(self.conv1(l2Feature)), inplace=True) #256
        # out2 = self.conv2(out1)
        # # w, b = out2[:, :64, :, :], out2[:, 64:, :, :]
        # l2Feature = F.relu(out2[:, :64, :, :] * out1 + out2[:, 64:, :, :], inplace=True)

        # l1Feature = l1 + F.interpolate(l2Feature, size=l1.size()[2:], mode='bilinear', align_corners=True)
        # out1 = F.relu(self.bn1(self.conv1(l1Feature)), inplace=True) #256
        # out2 = self.conv2(out1)
        # # w, b = out2[:, :64, :, :], out2[:, 64:, :, :]
        # l2Feature = F.relu(out2[:, :64, :, :] * out1 + out2[:, 64:, :, :], inplace=True)
...
class MLFA(nn.Module):
    ...
    def forward(self, x1, x2,x3):
        ...
        # x123features = self.feature_selection_end(x123features)
        # Concatefused_output = x123features
...
class FANet(nn.Module):
    def __init__(self, num_classes,padding_size,blend):
        ...
        # self.PFFusion_tan = PFA_(self.inter_channel, self.inter_channel,padding_size,blend)
    ...
    def forward(self, x):
        ...
        tan0_FPN_feature_list = self.FPN(tanfeatures_0)
        tan1_FPN_feature_list = self.FPN(tanfeatures_1)
        tan2_FPN_feature_list = self.FPN(tanfeatures_2)
        tan3_FPN_feature_list = self.FPN(tanfeatures_3)
        tan4_FPN_feature_list = self.FPN(tanfeatures_4)
        tan5_FPN_feature_list = self.FPN(tanfeatures_5)
        tan6_FPN_feature_list = self.FPN(tanfeatures_6)
        tan7_FPN_feature_list = self.FPN(tanfeatures_7)
        tan8_FPN_feature_list = self.FPN(tanfeatures_8)
        tan9_FPN_feature_list = self.FPN(tanfeatures_9)
        tan10_FPN_feature_list = self.FPN(tanfeatures_10)
        tan11_FPN_feature_list = self.FPN(tanfeatures_11)
        tan12_FPN_feature_list = self.FPN(tanfeatures_12)
        tan13_FPN_feature_list = self.FPN(tanfeatures_13)
        tan14_FPN_feature_list = self.FPN(tanfeatures_14)
        tan15_FPN_feature_list = self.FPN(tanfeatures_15)
        tan16_FPN_feature_list = self.FPN(tanfeatures_16)
        tan17_FPN_feature_list = self.FPN(tanfeatures_17)
        tan18_FPN_feature_list = self.FPN(tanfeatures_18)
        tan19_FPN_feature_list = self.FPN(tanfeatures_19)

        
        # l4_tanfeatures = [tan0_FPN_feature_list,tan1_FPN_feature_list,tan2_FPN_feature_list,tan3_FPN_feature_list,tan4_FPN_feature_list,
        #                 tan5_FPN_feature_list,tan6_FPN_feature_list,tan7_FPN_feature_list,tan8_FPN_feature_list,tan9_FPN_feature_list,
        #                 tan10_FPN_feature_list,tan11_FPN_feature_list,tan12_FPN_feature_list,tan13_FPN_feature_list,tan14_FPN_feature_list,
        #                 tan15_FPN_feature_list,tan16_FPN_feature_list,tan17_FPN_feature_list,tan18_FPN_feature_list,tan19_FPN_feature_list]
        ...
        # fused_l4feature = self.PFFusion_tan([equi_FPN_feature_list[3], l4_tanfeatures])
        # fused_l3feature = self.PFFusion([equi_FPN_feature_list[2]])
        # fused_l2feature = self.PFFusion([equi_FPN_feature_list[1]])
        # fused_l1feature = self.PFFusion([equi_FPN_feature_list[0]])
